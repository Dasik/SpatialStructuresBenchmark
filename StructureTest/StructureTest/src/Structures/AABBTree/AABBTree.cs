...
public class AABBTree<T> : IEnumerable<T>
{
    ...
    public Node<T> Add(FloatWithSizeMath.FloatWithSize[] insertedAABB, T data)
    {
        return InsertLeaf(data, insertedAABB[0].MinVal, insertedAABB[1].MinVal, insertedAABB[0].Size, insertedAABB[1].Size);
    }
    ...
    /// </summary>
    /// <returns>The index of the inserted node.</returns>
    public Node<T> InsertLeaf(T data, float x, float y, float width = 0, float height = 0)
    {
        // create new node and fatten its aabb
        var leafNode = new Node<T>(x, y, width, height, data, null);
        ...
        var newParent = new Node<T>(combinedAABB.x, combinedAABB.y, combinedAABB.width, combinedAABB.height, default(T), oldParent);
    ...
    public bool UpdateLeaf(Node<T> leafNode, float x, float y, float width = 0, float height = 0, float dx = 0, float dy = 0)
    {
        ...
        var newNode = InsertLeaf(data, x, y, width, height);
    ...
    public void Rebuild()
    {
        ...
        //    var parent = pool.get(aabb.x, aabb.y, aabb.width, aabb.height, default(T), null, getNextId());
        //    parent.left = left;
        //    parent.right = right;
    ...
    public List<T> FindValuesAt(FloatWithSizeMath.FloatWithSize[] targetAABB, bool strictMode = false, AdditionalCheckDelegate callback = null)
    {
    ...
    public List<T> Query(float x, float y, float width = 0, float height = 0, bool strictMode = false, List<T> into = null, AdditionalCheckDelegate additionalCheck = null)
    {
        ...
        while (stack.Count > 0)
        {
            var node = stack.Pop();
            //cnt++;
            if (!queryAABB.overlaps(node.aabb)) { continue;}
    ...
    {
        return Query(x, y, 0, 0, false, into, additionalCheck);
    }
	 ...
	 * @return A list of all the objects found (or `into` if it was specified).
	 */
    public List<T> RayCast(float fromX, float fromY, float toX, float toY, List<T> into, AdditionalCheckDelegate additionalCheck)
    {
        var res = into != null ? into : new List<T>();
        if (_root == null)
        {
        ...
        Query(rayAABB.x, rayAABB.y, rayAABB.width, rayAABB.height, false, null, (node) =>
        {
            var aabb = node.aabb;
    ...
    public T RemoveAt(FloatWithSizeMath.FloatWithSize[] AABB)
    {
        return default(T);
    }
