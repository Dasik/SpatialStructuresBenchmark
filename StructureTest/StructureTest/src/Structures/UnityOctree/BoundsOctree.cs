...
public class BoundsOctree<T> : IEnumerable<T>
{
	...
	public IEnumerator<T> GetEnumerator()
	{
		...
		var stack = new Stack<BoundsOctreeNode<T>>();
		if (rootNode == null)
			yield break;
		stack.Push(rootNode);
		while (stack.Count > 0)
		{
			...
			foreach (var o in currentElement.objects)
				yield return o.Obj;
			if (currentElement.children!=null)
			foreach (var node in currentElement.children)
			{
				if (node != null)
					stack.Push(node);
			}
